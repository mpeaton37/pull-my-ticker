...
class StockAnalyzer:
    def __init__(self, symbols, start_date, end_date):
        ...
        self.data = {}
        self.fundamental_data = {}  # Separate dictionary for fundamental data
        self.blacklist = ['ZTS']
        self.targets = {}  # Analyst price targets  

    @staticmethod
    def _sanitize_table_name(symbol):
                    ...
                    for db_col, class_key in column_mapping.items():
                        if db_col in fund_data.columns:
                            try:
                                ...
                                print(f"Warning: Could not load {db_col} for {symbol}: {str(e)}")
            else:
                print("Warning: fundamental_data table not found. Initializing with empty values.")
                for symbol in symbols:
                    analyzer.fundamental_data[symbol] = {
                        'PE_Ratio': None,
                        'Dividend_Yield': None,
    ...
    def export_to_sqlite(self, db_path):
        ...
        try:
            ...
            # Create summary data
            summary_data = []
            logger.info("Exporting data to SQLite.  Blacklist: {}".format(self.blacklist)) 
            ...
            for symbol, data in self.fundamental_data.items():
                data['symbol'] = symbol
                data['last_update'] = self.current_time.isoformat()
                data['target_mean'] = self.targets[symbol].get('mean')
                data['target_high'] = self.targets[symbol].get('high')
                data['target_low'] = self.targets[symbol].get('low')

                fundamental_records.append(data)
            ...
            print(f"Dataframe shape: {summary_df.shape if 'summary_df' in locals() else 'N/A'}")
        except Exception as e:
            print(f"Error exporting to database: {str(e)}")
    ...
    def fetch_market_data(self,force_refresh=True, latest_only=False):
    ...
    def add_advanced_indicators(self):
        for symbol, df in self.data.items():
            ...
            try:
                ...
                logging.error(f"Error calculating advanced indicators for {symbol}: {str(e)}")
                continue
    ...
    def export_to_excel(self, filename):
        ...
        try:
            ...
            print(f"Error: Unable to write to {filename}. Please ensure the file is not open in another program.")
        except Exception as e:
    ...
    def read_from_sqlite(self, db_path, symbol=None):
        """
        ...
        try:
            ...
            if conn:
                conn.close()
    def load_for_notebook(self, symbol=None, from_db=True):
        """Load data for Jupyter: Prefer DB for offline, fetch if needed."""
        if from_db and os.path.exists('stock_analysis.db'):
            return self.read_from_sqlite('stock_analysis.db', symbol)
        else:
    ...
    def visualize(self, symbol, plot_type='candlestick', interactive=True):
